...
with open('romenskii_energy.inc','w') as f:
    f.write(printing.fcode(e_internal, assign_to='result', source_format='free'))
    
with open('romenskii_entropy.inc','w') as f:
    f.write(printing.fcode(entropy, assign_to='result', source_format='free'))

with open('romenskii_stress.inc','w') as f:
    for i in range(1,4):
        for j in range(1,4):
            f.write(printing.fcode(sigma[i-1,j-1], assign_to="result(%d,%d)" % (i,j), source_format='free'))
            f.write("\n\n")

for i in range(1,4):
    for j in range(1,4):
        with open('romenskii_dsigma%d%d_dG.inc' % (i,j), 'w') as f:
            for p in range(1,4):
                for q in range(1,4):
                    f.write(printing.fcode(dsigma_dG[i-1,j-1][p-1,q-1], assign_to="result(%d,%d)" % (p,q), source_format='free'))
                    f.write("\n\n")

with open('romenskii_dsigma_dS.inc','w') as f:
    for i in range(1,4):
        for j in range(1,4):
            f.write(printing.fcode(dsigma_dS[i-1,j-1], assign_to="result(%d,%d)" % (i,j), source_format='free'))
            f.write("\n\n")
